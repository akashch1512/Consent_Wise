...
from app.integrations.summary import generate_summary
...
def _build_risk_signals(text: str, intent: str) -> list[RiskSignal]:
    ...
    if any(
        keyword in lower_text for keyword in ["double every second", "urgent", "immediately", "now"]
    ):
        signals.append(
            RiskSignal(
    ...
    if any(
        keyword in lower_text for keyword in ["owe", "debt", "pay", "payment", "subscribe", "buy"]
    ):
        signals.append(
            RiskSignal(
...
def _build_recommended_action(danger_score: int, risk_signals: list[RiskSignal]) -> str:
    ...
    if danger_score >= 40:
        return (
            "Proceed carefully. Review fees, obligations, and data-sharing terms before continuing."
        )
...
def _build_login_guidance(login_safety: str, reputation_score: int, danger_score: int) -> str:
    ...
    if normalized == "unsafe" or reputation_score < 40 or danger_score >= 75:
        return (
            "Not safe to log in right now. Avoid entering credentials until the site is verified."
        )
...
async def analyze_extension_payload(
    ...
    await record_analysis(
        ...
        login_safety=record.login_safety,
